...
import { InMemoryRuleRepository } from '../repositories/in-memory-rule-repository.ts'
...
describe('deleteCategoryAndReassign', () => {
  ...
  it('deletes the category itself', async () => {
    ...
    await deleteCategoryAndReassign(groceries.id, await transactions.list(), { categories, transactions, rules })
  ...
  it('removes rules that targeted the deleted category, leaving others untouched', async () => {
    const { categories, transactions, rules, groceries } = await setUp()
    await rules.create({ pattern: 'WHOLEFDS', categoryId: groceries.id })
    const otherRule = await rules.create({ pattern: 'NETFLIX', categoryId: 'category-entertainment' })

    await deleteCategoryAndReassign(groceries.id, await transactions.list(), { categories, transactions, rules })

    expect(await rules.list()).toEqual([otherRule])
  })
})
